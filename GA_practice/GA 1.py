...
pop_size = 50 #做50個
pop_len = 22
crover_rate = 0.25 #有0.5的機率會做crossover
mu_rate = 0.01 #有0.05的機率會做mutation
iteration = 150
x_sum = 0
maxi = 0
...
f_sample = np.zeros((pop_size,1))
fs_range = np.zeros((pop_size,2))
q = np.zeros((pop_size+1,1))
...
for i in range (iteration):
    ...
    for fre in range (pop_size): 
        ...
        #f_sample[fre,0]  = f[fre,0] + 1 
#==============================================================================
#         if(best_fs < f[fre,0]): #假如best_fs小於f(i)
#             # index of best fs
#             best_fs_i = x[fre,0]                #將fre這個index指給best_fs_i做紀錄，表示第幾個為最好的答案
#             # best fs value
#             best_fs = f[fre,0]      #將f(i)指給best_fs
...
#     for fre in range (pop_size):
#         for dec in range (pop_len):
#             x_ten[fre] = x_ten[fre] + x_bin[fre,dec] * (2**dec)
